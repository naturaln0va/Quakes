...
class PersistentController
{
    ...
    private static let worldContextName = "WorldQuakes"
    
    // MARK: - World Model
    lazy var worldMoc: NSManagedObjectContext = {
        ...
        let storeURL = documentsDirectory.URLByAppendingPathComponent("\(worldContextName).sqlite")
    ...
    func deleteWorldQuakes(quakesToDelete: [Quake]) {
        for quake in quakesToDelete {
            worldMoc.deleteObject(quake)
        }
        
        attemptSave()
    }
    
    func updateWorldQuakeWithID(identifier: String, withNearbyCities cities: [ParsedNearbyCity]?, withCountry country: String?) {
        do {
            if let quakeToUpdate = try Quake.singleObjectInContext(worldMoc, predicate: NSPredicate(format: "identifier == %@", identifier), sortedBy: nil, ascending: false) {
    ...
    func saveWorldQuakes(parsedQuake: [ParsedQuake]) {
        for quake in parsedQuake {
            do {
                if let _ = try Quake.singleObjectInContext(worldMoc, predicate: NSPredicate(format: "identifier == %@", quake.identifier), sortedBy: nil, ascending: false) {
                    continue
                }
            }
                
            catch {
                print("Failed to get a single object from the managed context in the world moc.")
            }
            
            guard let dataToSave = NSEntityDescription.insertNewObjectForEntityForName(Quake.entityName(), inManagedObjectContext: worldMoc) as? Quake else {
                fatalError("Expected to insert and entity of type 'Quake' in the world moc.")
            }
    ...
    func attemptSave() {
        if worldMoc.hasChanges {
            worldMoc.performBlockAndWait { [unowned self] in
                do {
                    try self.worldMoc.save()
                }
